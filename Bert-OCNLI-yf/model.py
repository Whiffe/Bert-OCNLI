...
# 读ocnli和snli，data_dir是数据集的路径
def read_ocnli_snli(data_dir, trainval_name, snli_path, isTest=False):
    # 将ocnli解析为前提、假设、标签
    # label_map是标签映射，0、1、2代表三类，3代表无法分类（或者应该去除的数据）。
    label_map = {'entailment':0, 'neutral':1, 'contradiction':2, '-': 3}
    file_name = os.path.join(data_dir, trainval_name)
    ...
    with open(file_name, 'r', encoding='utf-8') as f:
        ...
        if not isTest:
            ...
            with open(snli_path, 'r', encoding='utf-8') as f:
                for line in f:
                    json_obj = json.loads(line)
                    row = json.dumps(json_obj, ensure_ascii=False, indent=2)
                    row = json.loads(row)
                    data.append(
                        (
                            (row['sentence1'], row['sentence2']),
                            label_map[row['gold_label']]
                        )
                    )
...
# 读ocnli和snli，data_dir是数据集的路径
def read_ocnli_snli2(data_dir, trainval_name, snli_path, isTest=False):
    # 将ocnli解析为前提、假设、标签
    # label_map是标签映射，0、1、2代表三类，3代表无法分类（或者应该去除的数据）。
    label_map = {'entailment':0, 'neutral':1, 'contradiction':2, '-': 3}
    file_name = os.path.join(data_dir, trainval_name)
...
class OCNLI_Dataset(Dataset):
    def __init__(
        ...
        device,
        max_length=128,
        pretrain_model_name='hfl/bert-base-chinese'
    ):
