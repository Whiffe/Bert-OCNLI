...
def train(model, train_dataset, val_dataset, configs, device, wandb):
    ...
    train_data = OCNLI_Dataset(train_dataset, device, configs.max_length)
    val_data = OCNLI_Dataset(val_dataset, device, configs.max_length)
    ...
    for epoch in range(configs.epochs):
        ...
        这个方法会告诉 PyTorch 你正在训练模型，因此它会启用 Dropout 和 BatchNorm 的训练行为。
        例如，Dropout 层在训练时会随机丢弃一部分神经元，而在测试时则不会。
        
        model.eval(): 将模型设置为评估模式。
        这个方法会告诉 PyTorch 你正在评估（验证或测试）模型，
        因此它会禁用 Dropout 和使用 BatchNorm 的移动平均值和方差。

        缺少 model.train() 和 model.eval() 可能会导致模型在训练和评估过程中行为不一致，
        
        训练过程中的损失计算
        在训练过程中，我们通常会在每个批次上计算损失并进行梯度更新。

        批次损失：
            我们计算每个批次的平均损失。
        梯度计算和参数更新：
            我们使用每个批次的平均损失来计算梯度并更新模型参数。

        在训练过程中，损失计算和参数更新是逐批次进行的，
        因此直接使用每个批次的平均损失进行反向传播和参数更新即可，
        不需要考虑整个训练集的总损失。
        ...
        for input_ids, attention_mask, token_type_ids, labels in tqdm(train_dataloader):
            ...
            output = model(input_ids, attention_mask, token_type_ids) # 通过模型得到输出
        ...
        '''
        验证过程中的损失计算
        在验证过程中，我们希望计算整个验证集的平均损失。
        因此，我们需要累积每个批次的总损失，然后除以整个验证集的样本数量。

        计算当前批次的总损失：
            将当前批次的平均损失乘以批次大小 (input_ids.size(0))。
        累积总损失：
            将每个批次的总损失累积起来。
        计算平均损失：
            在所有批次的损失累积完毕后，除以验证集的总样本数，得到整个验证集的平均损失。
        '''
        # 设置模型为评估模式
        model.eval()
        # 不需要计算梯度
        ...
        print('val_loss.item():',val_acc)
        model.train()

def test(model, test_dataset, configs, device):
    test_data = OCNLI_Dataset_test(test_dataset, device, configs.max_length)
    test_dataloader = DataLoader(test_data, batch_size=32, shuffle=False)

    state_dict = torch.load(os.path.join(configs.checkpoint_dir, f'epoch_{configs.epochs + 1}.pt'))
